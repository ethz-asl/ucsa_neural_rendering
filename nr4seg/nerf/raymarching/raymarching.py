...
class _composite_rays_train_semantics(Function):

    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, local_semantics, deltas, rays,
                num_semantics_classes):
        """composite rays' rgbs, according to the ray marching formula.
        Args:
            rgbs: float, [M, 3]
            sigmas: float, [M,]
        ...
        sigmas = sigmas.contiguous()
        rgbs = rgbs.contiguous()
        local_semantics = local_semantics.contiguous()
        ...
        depth = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, 3, dtype=sigmas.dtype, device=sigmas.device)
        semantics = torch.empty(N,
                                num_semantics_classes,
                                dtype=sigmas.dtype,
                                device=sigmas.device)

        _backend.composite_rays_train_semantics_forward(
            sigmas,
            rgbs,
            local_semantics,
            deltas,
        ...
        return weights_sum, depth, image, semantics

    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights_sum, grad_depth, grad_image, grad_semantics):

        # NOTE: grad_depth is not used now! It won't be propagated to sigmas.

        grad_weights_sum = grad_weights_sum.contiguous()
        grad_image = grad_image.contiguous()
        grad_semantics = grad_semantics.contiguous()
        ...
        grad_sigmas = torch.zeros_like(sigmas)
        grad_rgbs = torch.zeros_like(rgbs)
        grad_local_semantics = torch.zeros_like(local_semantics)

        _backend.composite_rays_train_semantics_backward(
            grad_weights_sum,
            grad_image,
            grad_semantics,
            sigmas,
            ...
            grad_sigmas,
            grad_rgbs,
            grad_local_semantics,
        )

        return grad_sigmas, grad_rgbs, grad_local_semantics, None, None, None


composite_rays_train_semantics = _composite_rays_train_semantics.apply
...
class _composite_rays_semantics(Function):
    ...
    def forward(
        ...
        In-place Outputs:
            ...
            image: float, [N, 3], the RGB channel (after multiplying alpha!)
        """
        _backend.composite_rays_semantics(
            n_alive,
            n_step,
            rays_alive,
            rays_t,
...
composite_rays_semantics = _composite_rays_semantics.apply
