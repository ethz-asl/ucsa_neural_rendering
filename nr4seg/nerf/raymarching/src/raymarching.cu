...
// template <typename scalar_t>
// __global__ void kernel_composite_rays_semantics(
//     const uint32_t n_alive, 
//     const uint32_t n_step, 
//     const int* __restrict__ rays_alive, 
//     scalar_t* rays_t, 
//     const scalar_t* __restrict__ sigmas, 
//     const scalar_t* __restrict__ rgbs, 
//     const scalar_t* __restrict__ local_semantics, 
//     const scalar_t* __restrict__ deltas, 
//     scalar_t* weights_sum, scalar_t* depth, scalar_t* image, scalar_t* semantics,
// ) {
//     const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
//     if (n >= n_alive) return;
...
// void composite_rays_semantics(const uint32_t n_alive, const uint32_t n_step, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor rgbs, at::Tensor local_semantics, at::Tensor deltas, at::Tensor weights_sum, at::Tensor depth, at::Tensor image, at::Tensor semantics);
//     static constexpr uint32_t N_THREAD = 256;
//     AT_DISPATCH_FLOATING_TYPES_AND_HALF(
//     image.scalar_type(), "composite_rays_semantics", ([&] {
//         kernel_composite_rays<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), local_semantics.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), semantics.data_ptr<scalar_t>());
//     }));
// }
